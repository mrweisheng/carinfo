...
- **缺维从分母去掉**（不是记 0 分）。热度只有 44.8% 覆盖、车况 28.0%，
  记 0 分等于给一半以上的车无差别扣分，排序会被"数据齐全度"主导。
- 无车型目标时（"50 万以内的 7 座"）匹配维整维缺席，不给常数分。
- 性价比锚点：中位价（ratio=1.00）得 0.70 分而不是 0.5 —— 与同款同价是正常交易。
- **前五维 = 旧权重 × 0.8，腾出 0.20 给后加的「贴合度」。** 这不是随手调的：缺维从分母
  去掉之后，没设「左右」的查询 near 维缺席，
  `Σ(0.8·wᵢ·sᵢ) / Σ(0.8·wᵢ) = Σ(wᵢ·sᵢ)` —— 与旧口径**数学上完全等价**，
  于是新维度对既有查询零影响，评测里的任何差异都能 100% 归因到新功能。
  `test_search.py [9][10]` 把这条锁死了，改权重前先看那两条断言。
...
10. **FileLock stale_after 已改为 2 小时 + 每页 `touch()`**（原为固定 8h < 整轮 12h，跨夜会被误判 stale）。
    ...
    ⚠️ **若哪天把 `page_hook=self.lock.touch` 删了**（或 `scrape_vehicle_type` 不再回调），阈值必须调回 > 单轮最坏耗时
